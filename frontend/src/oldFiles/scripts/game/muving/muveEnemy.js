...
function muveUp(trajectory) {
  let divUser = document.querySelector(`[data-hero="enemy"]`);
  let coordUser = divUser.getBoundingClientRect();

  let divSquareUser = document.querySelector(`[data-player="enemy"]`);
  ...
  let idInterval = setInterval(function () {
    ...
    if (counter >= pathLength) {
      destinationSquare.dataset.player = "enemy";
      divSquareUser.dataset.player = "";
      trajectory.shift();
      muveEnemy(trajectory);
...
export function muveEnemy(trajectory) {
  if (isMyMuve()) return;
